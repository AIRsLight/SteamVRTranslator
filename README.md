...
- 选择框、截图、流式结果和扩展窗口共享一个 D3D11 设备；每个持久对象都是独立的绝对空间 Quad Overlay，拥有自己的句柄、变换、纹理和显隐生命周期。
...
- 抓握键让截图、结果和 WPF 窗口完整跟随手部平移及旋转；右摇杆按下全局显示/隐藏空间对象，上下滚动当前触碰的结果或 WPF 控件。
...
10. 按下右摇杆全局隐藏或重新显示空间对象；新的截图会自动重新显示。触碰或持有结果层时，右摇杆上下平滑滚动长文本；方向不符合习惯时，在控制窗口勾选“反转右摇杆滚动方向”。
...
默认操作键：Touch、Index 和 WMR 使用左手摇杆按下进入选择或翻译当前截图；Vive 手柄使用左触控板。左右抓握键移动空间对象，另一只手的扳机点击空间窗口，右手摇杆或触控板负责滚动和全局显隐。绑定可在 SteamVR 中修改。桌面默认热键为 F8。
